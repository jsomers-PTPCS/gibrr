...
export function PostComments({ postId }: { postId: string }) {
  ...
  useEffect(() => {
    // A single, always-fast fetch — GET /posts/:postId/comments
    // (routes/comments.ts) now answers straight from the DB regardless
    // of whether this is a remote post, kicking off any live re-sync of
    // a stale remote reply thread in the background instead of making
    // this request wait on it. The old two-phase cached-then-synced
    // dance existed only to paper over that live sync taking real
    // seconds inline; it's no longer needed now that the slow part
    // moved off this request entirely.
    refetchComments();
    getMe()
      .then(setMe)
      .catch(() => setMe(null));
